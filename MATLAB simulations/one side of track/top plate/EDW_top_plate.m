...
ro = 0.141; % outer radius of rotor [m]
ri = 0.04788; % inner radius of rotor [m]
Br = 1.42; % magnet remanence [T]
mu_r = 1.08; % relative permeability

P = 4; % pole pairs
slip = [-30:5:45]; % slip speed [m/s]
vx = 135; % translational speed of the wheel [m/s]
...
for ii = 1:length(w_angle)
    for jj = 1:length(slip)
        ...
        for i = 1:length(eta)
            [Y1(i), Y2(i), Y3(i)] = EDW_calc_series_top_plate(eta(i),mu_o,ro,ri,Br,mu_r,P,rspeed(jj),vx,sigma,b,g,num_of_wheels,d,w_angle(1)); % calculating the integrand
        end   
